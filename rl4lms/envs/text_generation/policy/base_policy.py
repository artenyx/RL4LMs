...
class LMActorCriticPolicy(BasePolicy):
    def __init__(
        ...
        self._build_model_heads(model_name, ref_model_name)
    ...
    def generate_from_ref(
        ...
        gen_output = unwrap_model(self._ref_model).generate(
            inputs=input_ids.to(self.get_policy_first_device()),  # THIS LINE MIGHT BREAK
            attention_mask=attention_mask.to(self.get_policy_first_device()),  # THIS LINE MIGHT BREAK
            return_dict_in_generate=True,
            output_scores=True,
            **generation_kwargs_,
        )
