...
if [[ "$exp" == init_beta ]]; then
  ...
  if [[ "$kl_type" == full_kl_2 ]]; then
    lr_list=(0.000001 0.000002 0.000003 0.000004)
    targ_kl_list=(1.2 1.4 1.6 1.8 2.0) #(0.8 1.0 1.2 1.4 1.6 1.8 2.0 2.2 2.4) #(0.5 0.8 1.0 1.2 1.4 1.6)
    ...
    lr_list=(0.0000006 0.0000007 0.0000008 0.0000009 0.000001)
  ...
  do
    ...
    do
      sbatch_params="-p $partition -C 48g slurm/single_exp.sh $task_name $base_model_name $exp_arg NONE $group $kl_type $off_policy targ_kl,lr,n_envs $targ_kl,$lr,40"
      echo "sbatch arguments: $sbatch_params"
      sbatch $sbatch_params
    done
  done
